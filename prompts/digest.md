...
headlines, high-scoring Hacker News stories, high-karma LessWrong posts, and new posts
from followed blogs.
...
- **Topics in the News** (from the Wikipedia data): these are Wikipedia's curated top
  headlines, each already a one-sentence blurb. List each as a single bullet, lightly
  cleaned up for readability, linking the Wikipedia article URL provided.
  Format: `- [headline text](url)`. Do not invent or add detail beyond the blurb.
